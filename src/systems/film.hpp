...
// concept ThreeD_Lattice = std::same_as<typename lattice_t::sizes_t,
// qss::lattices::three_d::sizes_t>;
...
struct film final : public lattice_t {
    ...
    using xy_border_condition = typename qss::borders_conditions::
        periodic<typename lattice_t::coords_t::size_type, typename lattice_t::sizes_t::size_type>;
    using z_border_condition = typename qss::borders_conditions::
    ...
    std::pair<typename lattice_t::value_t::magn_t, unsigned int>
    fill_plane(unsigned int z, const typename lattice_t::value_t& value)
    {
        ...
        for (const auto& pattern : patterns) {
            ...
            for (typename lattice_t::coords_t::size_type i{0}; i < this->sizes.x; ++i) {
                ...
                for (typename lattice_t::coords_t::size_type j{0}; j < this->sizes.y; ++j) {
                    coord.x = j;
                    const auto exists = qss::borders_conditions::use_border_conditions<
                        z_border_condition,
                        z_border_condition,
...
copy_structure(const film<lattice_t<old_spin_t>>& original) noexcept
{
    ...
    return film<lattice_t<spin_t>>{
        copy_structure<spin_t>(dynamic_cast<const lattice_t<old_spin_t>&>(original)), original.J};
}
...
get_closest_neigbour_from_upper_film(
...
// requires std::is_same_v<typename lattice_t::coords_t, qss::lattices::three_d::fcc_coords_t>
[[nodiscard]] std::optional<qss::lattices::three_d::fcc_coords_t>
get_closest_neigbour_from_lower_film(
    const film<lattice_t>& other, const qss::lattices::three_d::fcc_coords_t& coord) noexcept
{
    auto result = coord;
    if (coord.z > 0 || coord.z < other.sublattices_sizes[coord.w].z - 1) {
